...
export const handler = async (event, context) => {
  const { name, email, address, neighborhood, city, state, phone, zip } = JSON.parse(event?.body);
  if (!name || !email || !validateEmail(email) || !address || !neighborhood || !city || !state || !phone || !zip ) 
  ...
  try {
    ...
    const resultData = await Profiles.findOne({ where: { userId: user.id } });
