...
describe("Persons API - Delete", () => {
  ...
  test("Should fail with invalid ID", async () => {
    const res = await handler(await createEvent(LambdaTypes.Delete, {}, Tokens.Valid, '53ba167d-48eb-49f8-a881-b6984132116a'));
    expect(res.statusCode).toEqual(404);
    expect(JSON.parse(res.body).message).toBe("Registro não encontrado!");
  });

  test("Should success", async () => {
    const res = await handler(await createEvent(LambdaTypes.Delete, {}, Tokens.Valid, personID));
