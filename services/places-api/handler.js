...
export const handler = async (event) => {
  ...
  if (path === LambdaTypes.Active) return await active(user.tenant, event?.pathParameters, body.object);
  ...
  if (path === LambdaTypes.Employees) return await employees(user.tenant, event?.pathParameters, body.object);
  ...
  if (path === LambdaTypes.Managers) return await managers(user.tenant, event?.pathParameters, body.object);
  if (path === LambdaTypes.Update) return await update(user.tenant, event?.pathParameters, body.object);
