...
describe("Places API - Handler", () => {
  ...
  test("Should fail without authorizer", async () => {
    const res = await handler(await createEvent(LambdaTypes.Active, {}));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Autorização não encontrada!");
  });

  test("Should fail with user not found", async () => {
    const res = await handler(await createEvent(LambdaTypes.Active, {}, Tokens.NotFound));
    expect(res.statusCode).toEqual(404);
    expect(JSON.parse(res.body).message).toBe("Usuário não encontrado!");
  });

  test("Should fail with expired token", async () => {
    const res = await handler(await createEvent(LambdaTypes.Active, {}, Tokens.Expired));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Autorização não encontrada!");
  });
