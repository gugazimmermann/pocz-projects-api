...
const employeesList = [{id: "a5109879-c5c0-48d2-9f9b-f17450b89c7d"}, {id: "4e573fa5-00b3-45cf-bdf5-c88ddf8cbabe"}];

describe("Places API - Employees", () => {
  afterAll(() => { close() });

  test("Should fail without ID", async () => {
    const res = await handler(await createEvent(LambdaTypes.Employees, { employeesList }, Tokens.Valid, {}));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Dados inválidos!");
  });

  test("Should fail without body", async () => {
    const res = await handler(await createEvent(LambdaTypes.Employees, {}, Tokens.Valid, { id: placeID }));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Dados inválidos!");
  });

  test("Should fail with invalid ID", async () => {
    const res = await handler(await createEvent(LambdaTypes.Employees, { employeesList }, Tokens.Valid, { id: 'ae4e262e-545c-4165-ad6c-068a28008c59' }));
    expect(res.statusCode).toEqual(404);
    expect(JSON.parse(res.body).message).toBe("Registro não encontrado!");
  });

  test("Should success", async () => {
    const res = await handler(await createEvent(LambdaTypes.Employees, { employeesList }, Tokens.Valid, { id: placeID }));
