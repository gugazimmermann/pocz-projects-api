...
describe("Places API - Update", () => {
  ...
  test("Should fail without name", async () => {
    const res = await handler(await createEvent(LambdaTypes.Update, {...place, name: null}, Tokens.Valid, placeId));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Dados inválidos!");
  });

  test("Should fail without zip", async () => {
    const res = await handler(await createEvent(LambdaTypes.Update, {...place, zip: null}, Tokens.Valid, placeId));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Dados inválidos!");
  });

  test("Should fail without address", async () => {
    const res = await handler(await createEvent(LambdaTypes.Update, {...place, address: null}, Tokens.Valid, placeId));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Dados inválidos!");
  });

  test("Should fail without city", async () => {
    const res = await handler(await createEvent(LambdaTypes.Update, {...place, city: null}, Tokens.Valid, placeId));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Dados inválidos!");
  });

  test("Should fail without state", async () => {
    const res = await handler(await createEvent(LambdaTypes.Update, {...place, state: null}, Tokens.Valid, placeId));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Dados inválidos!");
  });

  test("Should fail with invalid email", async () => {
    const res = await handler(await createEvent(LambdaTypes.Update, {...place, email: 'a'}, Tokens.Valid, placeId));
    expect(res.statusCode).toEqual(400);
    expect(JSON.parse(res.body).message).toBe("Dados inválidos!");
  });

  test("Should fail with invalid id", async () => {
    const res = await handler(await createEvent(LambdaTypes.Update,place, Tokens.Valid, "2e830d10-2205-4e3d-ae15-e6c96e1a297e"));
    expect(res.statusCode).toEqual(404);
    expect(JSON.parse(res.body).message).toBe("Registro não encontrado!");
  });

  test("Should success", async () => {
    const res = await handler(await createEvent(LambdaTypes.Update, place, Tokens.Valid, placeId));
